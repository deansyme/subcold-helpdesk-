...
import { Globe, Plus, Save, X, ChevronDown, ChevronUp } from 'lucide-react'
...
export function CategoryTranslations({
  ...
  async function fetchTranslations() {
    try {
      const response = await fetch(`/api/admin/translations?type=category&id=${categoryId}`)
      if (response.ok) {
  ...
  function startEditing(locale: string, existingTranslation?: CategoryTranslation) {
  ...
  async function deleteTranslation(locale: string) {
    ...
    try {
      const response = await fetch(
        `/api/admin/translations?type=category&id=${categoryId}&locale=${locale}`,
        { method: 'DELETE' }
      )
      ...
      {isExpanded && (
        <div className="p-4 space-y-4">
          ...
          <div className="bg-blue-50 p-3 rounded-lg text-sm">
            ...
            <p className="text-blue-700">{originalName}</p>
          </div>
            ...
            return (
              <div key={translation.id} className="border border-gray-200 rounded-lg p-4">
                ...
                {isEditing ? (
                  ...
                  <p className="text-gray-600">{translation.name}</p>
                )}
              </div>
            )
          })}
